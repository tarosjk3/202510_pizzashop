...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    if (!array_filter($errors)) {
        ...
        if ($is_image_uploaded) {
            $stmt = $db->prepare('UPDATE pizzas SET chef_name = ?, pizza_name = ?, toppings = ?, image = ? WHERE id = ?');
        } else {
            $stmt = $db->prepare('UPDATE pizzas SET chef_name = ?, pizza_name = ?, toppings = ? WHERE id = ?');
        }
        // 登録するデータの挿入
...
<div class="container">
    ...
    <h1 class="my-5 h4 text-center">ピザの更新</h1>

    <div class="row justify-content-center">
        <div class="col-md-8">
            <form action="update.php?id=<?= htmlspecialchars($_GET['id']); ?>" method="post" enctype="multipart/form-data">
                <div class="mb-3">
                ...
                <div class="text-center">
                    ...
                    <button type="submit" class="btn btn-primary" name="submit" value="1">更新する</button>
                </div>
            </form>
        </div>
    </div>
